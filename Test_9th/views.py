...
from .models import DefineClasses, GeneralInformation_9th, IndustryCategory, Interpretation_9th, Reports_9th, Questions, Test, DefinedGrate, ShowGrade
from .serializer import DefineClassesSerializer, GeneralInformation_9thSerializer, IndustryCategorySerializer, Reports_9thSerializer, TestSerializer, QuestionsSerializer, ShowGradeSerializer
...
def getTitle(request):
    que = GeneralInformation_9th.objects.all()
    serializer = GeneralInformation_9thSerializer(que, many=True)
...
def testResult(request):
    ...
    testV = Reports_9th.objects.filter(Q(industry=indData), Q(user=user))
    ...
    if testV:
        ...
        getUpdate = Reports_9th.objects.get(id=VID) 
        ...
        for i in obj:
            myGrade = i.grade
        interp = Interpretation_9th.objects.filter(grade = myGrade)
        for k in interp:
            interp_Id = k.id
        industryValue = IndustryCategory.objects.filter(industry=indData)
        for i in industryValue:
            industry_Grade = i.industry_Id
            Reports_9th.objects.filter(pk=VID).update(interpretatio = interp_Id, totalCount=CW, grade=myGrade, industry_Grade=industry_Grade)
    else:
        query = Reports_9th.objects.create(
            user = user,
        ...
        query.save()
        serializer = Reports_9thSerializer(query, many=False)
...
#     user = request.user
#     tickets = Reports_9th.objects.filter(Q(user=user)).annotate(total_price=F('totalCount'))
#     countIndstry = Reports_9th.objects.filter(Q(user=user)).count()
#     # print(countIndstry)
#     to = []
#     for t in tickets:
#         p = t.total_price
...
#     # print(to)
#     totalAmmount = sum(to)
#     serializer = Reports_9thSerializer(tickets, many=True)
...
def getResult(request):
    user = request.user
    data = Reports_9th.objects.filter(Q(user=user)).annotate(total_price=F('totalCount'))
    data2 = ShowGrade.objects.all()
    # data3 = GeneralInformation_9th.objects.all()
    context = {"mylist": data, "mylist1": data2}
    template = get_template('Test_9th/mytemplate.html')
    ...
    # print(to)
    totalAmmount = sum(to)
    serializer = Reports_9thSerializer(data, many=True)
    context = {'data':serializer.data, 'ind_1':totalAmmount, "countIndustry": countIndstry}
    # p = TestResult.objects.filter(user=user).delete()
    return Response(context)
...
def delReuslt(request):
    user = request.user
    Reports_9th.objects.filter(user=user).delete()
