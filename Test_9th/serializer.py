...
from .models import DefineClasses, IndustryCategory, Reports_9th, Test, Questions, ShowGrade, GeneralInformation_9th, Interpretation_9th
...
class Interpretation_9thSerializer(serializers.ModelSerializer):
    class Meta:
        model = Interpretation_9th
        fields = ['id', 'grade', 'interpretationTitle', 'YouCanDoPoint_1', 'YouCanDoPoint_2', 'YouCanDoPoint_3', 'YouCanDoPoint_4', 'YouCanDoPoint_5']

class Reports_9thSerializer(serializers.ModelSerializer):
    CreateAt = serializers.DateTimeField(format = "%B %d, %Y, %I:%M%p")
    interpretatio = Interpretation_9thSerializer(many=False, read_only=True)
    class Meta:
        model = Reports_9th
...
class GeneralInformation_9thSerializer(serializers.ModelSerializer):
    class Meta:
        model = GeneralInformation_9th
