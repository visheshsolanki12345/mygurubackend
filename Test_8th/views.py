...
from .models import DefineClasses, IndustryCategory, Reports_8th, Questions, Test, DefinedGrate, ShowGrade, GeneralInformation_8th, Interpretation_8th
from .serializer import DefineClassesSerializer, IndustryCategorySerializer, Reports_8thSerializer, TestSerializer, QuestionsSerializer, ShowGradeSerializer, GeneralInformation_8thSerializer
...
def getTitle(request):
    que = GeneralInformation_8th.objects.all()
    serializer = GeneralInformation_8thSerializer(que, many=True)
...
def testResult(request):
    ...
    testV = Reports_8th.objects.filter(Q(industry=indData), Q(user=user))
    
    if testV:
        course_qs = testV
        for course in course_qs:
            VID = course.id
        getUpdate = Reports_8th.objects.get(id=VID)
        if saveCount == 1:
        ...
        for i in obj:
            myGrade = i.grade
            interp = Interpretation_8th.objects.filter(grade = myGrade)
            for k in interp:
                interp_Id = k.id
        ...
        for i in industryValue:
            ...
            Reports_8th.objects.filter(pk=VID).update(interpretatio = interp_Id,totalCount=CW, grade=myGrade, industry_Grade=industry_Grade)
            saveCount = ""
    else:
        if saveCount == 1:
            query = Reports_8th.objects.create(
                user = user,
        ...
        query.save()
        serializer = Reports_8thSerializer(query, many=False)
...
#     user = request.user
#     tickets = Reports_8th.objects.filter(Q(user=user)).annotate(total_price=F('totalCount'))
#     countIndstry = Reports_8th.objects.filter(Q(user=user)).count()
#     # print(countIndstry)
#     to = []
#     for t in tickets:
#         p = t.total_price
...
#     # print(to)
#     totalAmmount = sum(to)
#     serializer = Reports_8thSerializer(tickets, many=True)
...
def getResult(request):
    user = request.user
    data = Reports_8th.objects.filter(Q(user=user)).annotate(total_price=F('totalCount'))
    data2 = ShowGrade.objects.all()
    # print('...............', data2)
    # data3 = GeneralInformation_9th.objects.all()
    context = {"mylist": data, "mylist1": data2}
    template = get_template('Test_8th/mytemplate.html')
    ...
    # print(to)
    totalAmmount = sum(to)
    serializer = Reports_8thSerializer(data, many=True)
    context = {'data':serializer.data, 'ind_1':totalAmmount, "countIndustry": countIndstry}
    # p = TestResult.objects.filter(user=user).delete()
    return Response(context)
...
def delReuslt(request):
    user = request.user
    Reports_8th.objects.filter(user=user).delete()
