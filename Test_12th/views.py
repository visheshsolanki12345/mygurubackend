...
from .models import DefineClasses, IndustryCategory, Reports_12th, Questions, Test, DefinedGrate, ShowGrade, GeneralInformation_12th, Interpretation_12th
from .serializer import DefineClassesSerializer, IndustryCategorySerializer, Reports_12thSerializer, TestSerializer, QuestionsSerializer, ShowGradeSerializer, GeneralInformation_12thSerializer
...
def getTitle(request):
    que = GeneralInformation_12th.objects.all()
    serializer = GeneralInformation_12thSerializer(que, many=True)
...
def testResult(request):
    ...
    testV = Reports_12th.objects.filter(Q(industry=indData), Q(user=user))
    ...
    if testV:
        ...
        getUpdate = Reports_12th.objects.get(id=VID) 
        ...
        for i in obj:
            myGrade = i.grade
        interp = Interpretation_12th.objects.filter(grade = myGrade)
        for k in interp:
            interp_Id = k.id
        industryValue = IndustryCategory.objects.filter(industry=indData)
        for i in industryValue:
            industry_Grade = i.industry_Id
            Reports_12th.objects.filter(pk=VID).update(interpretatio = interp_Id, totalCount=CW, grade=myGrade, industry_Grade=industry_Grade)
    else:
        query = Reports_12th.objects.create(
            user = user,
        ...
        query.save()
        serializer = Reports_12thSerializer(query, many=False)
...
#     user = request.user
#     tickets = Reports_12th.objects.filter(Q(user=user)).annotate(total_price=F('totalCount'))
#     countIndstry = Reports_12th.objects.filter(Q(user=user)).count()
#     # print(countIndstry)
#     to = []
#     for t in tickets:
#         p = t.total_price
...
#     # print(to)
#     totalAmmount = sum(to)
#     serializer = Reports_12thSerializer(tickets, many=True)
...
def getResult(request):
    user = request.user
    data = Reports_12th.objects.filter(Q(user=user)).annotate(total_price=F('totalCount'))
    data2 = ShowGrade.objects.all()
    # print('...............', data2)
    # data3 = GeneralInformation_9th.objects.all()
    context = {"mylist": data, "mylist1": data2}
    template = get_template('Test_12th/mytemplate.html')
    ...
    # print(to)
    totalAmmount = sum(to)
    serializer = Reports_12thSerializer(data, many=True)
    context = {'data':serializer.data, 'ind_1':totalAmmount, "countIndustry": countIndstry}
    # p = TestResult.objects.filter(user=user).delete()
    return Response(context)
...
def delReuslt(request):
    user = request.user
    Reports_12th.objects.filter(user=user).delete()
